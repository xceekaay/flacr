...
// Single place to talk to the renderer. Guards against a window that has
// already been destroyed (a 4h check can land long after the window is gone).
...
function setupAutoUpdater() {
  try {
    ...
    autoUpdater = au
    const settings = loadFileSettings() || {}
...
async function performUpdateCheck() {
  ...
  try {
    ...
    sendUpdateStatus('error', { message: e?.message || String(e) })
  }
}

// Bound exactly once, during setup. Re-binding on every check wiped the
// updater library's own internal listeners too.
function bindUpdaterEvents() {
  ...
  autoUpdater.on('download-progress',    prog  => sendUpdateStatus('downloading', { percent: Math.round(prog?.percent || 0) }))
...
function createWindow() {
  ...
  // Discord fetches this URL from its own servers and shows it on a public
  // profile. Only allow plain https on a publicly reachable host: a private or
  // local address would both leak the user's network layout and fail to load.
  function isSafeDiscordImageUrl(urlStr) {
    ...
    // Bare hostname with no dot is a LAN name, not a public domain.
    if (!host.includes('.')) return false
    return true
  }

  // ── Public cover hosting — Discord fetches artwork from its own servers, so
  //    a private or LAN Jellyfin address can never load. When the cover URL is
  //    not publicly reachable, upload the image once to Litterbox (Catbox's
  //    anonymous service) and send Discord the hosted link instead. The local
  //    server address is never exposed. ──────────────────────────────────────
  const COVER_CACHE_PATH     = () => path.join(app.getPath('userData'), 'covers.json')
  const COVER_REUPLOAD_AFTER = 48 * 60 * 60 * 1000   // links last 72h, refresh early
  const COVER_RETRY_AFTER    = 10 * 60 * 1000        // wait before retrying a failure
  const hostedCovers  = new Map()   // coverUrl -> { url, uploadedAt }
  const failedCovers  = new Map()   // coverUrl -> last failure timestamp
  const pendingCovers = new Map()   // coverUrl -> in-flight upload promise

  try {
    const saved = JSON.parse(fs.readFileSync(COVER_CACHE_PATH(), 'utf8'))
    if (saved && typeof saved === 'object') {
      for (const [k, v] of Object.entries(saved)) {
        if (v && typeof v.url === 'string') hostedCovers.set(k, v)
      }
    }
  } catch { /* first run, no cache yet */ }

  function persistCoverCache() {
    const entries = [...hostedCovers.entries()].slice(-500)
    fs.promises.writeFile(COVER_CACHE_PATH(), JSON.stringify(Object.fromEntries(entries))).catch(() => {})
  }

  async function uploadCover(buffer, contentType) {
    const form = new FormData()
    form.append('reqtype', 'fileupload')
    form.append('time', '72h')
    form.append('fileToUpload', new Blob([buffer], { type: contentType }), 'cover')
    ...
    if (!/^https:\/\/litter\.catbox\.moe\//.test(url)) throw new Error('upload service returned an unexpected response')
    return url
  }

  async function getHostedCoverUrl(coverUrl) {
    if (!coverUrl || typeof coverUrl !== 'string') return null
    if (isSafeDiscordImageUrl(coverUrl)) return coverUrl   // already publicly reachable

    const cached = hostedCovers.get(coverUrl)
    if (cached && Date.now() - cached.uploadedAt < COVER_REUPLOAD_AFTER) return cached.url

    const failedAt = failedCovers.get(coverUrl)
    if (failedAt && Date.now() - failedAt < COVER_RETRY_AFTER) return null

    const pending = pendingCovers.get(coverUrl)
    if (pending) return pending

    const job = (async () => {
      try {
        // Only ever pull artwork from the user's own active server.
        if (!activeOrigin || !coverUrl.startsWith(activeOrigin)) throw new Error('cover is not from the active server')
        const headers = activeToken ? { 'X-Emby-Token': activeToken } : {}
        const res = await fetch(coverUrl, { headers, signal: AbortSignal.timeout(15000) })
        if (!res.ok) throw new Error(`cover fetch failed with status ${res.status}`)
        const contentType = res.headers.get('content-type') || 'image/jpeg'
        if (!contentType.startsWith('image/')) throw new Error('response is not an image')
        const buffer = Buffer.from(await res.arrayBuffer())
        if (!buffer.length || buffer.length > 10 * 1024 * 1024) throw new Error('cover size out of range')
        const url = await uploadCover(buffer, contentType)
        hostedCovers.set(coverUrl, { url, uploadedAt: Date.now() })
        failedCovers.delete(coverUrl)
        persistCoverCache()
        return url
      } catch (err) {
        failedCovers.set(coverUrl, Date.now())
        console.warn('[Discord] cover hosting skipped:', err.message)
        return null
      } finally {
        pendingCovers.delete(coverUrl)
      }
    })()
    pendingCovers.set(coverUrl, job)
    return job
  }

  ipcMain.on('discord-presence', async (event, payload) => {
    if (!isFromMainWindow(event)) return
    if (!payload.playing || !payload.title) { clearDiscordPresence(); return }

    let largeImageKey = 'flacr'
    if (payload.coverUrl) {
      const hosted = await getHostedCoverUrl(payload.coverUrl)
      if (hosted) largeImageKey = hosted
    }

    // Ensure startTimestamp is in seconds (Discord expects Unix epoch in seconds)
    let startTimestamp = undefined
    if (payload.startTimestamp) {
      startTimestamp = payload.startTimestamp > 99999999999
        ? Math.floor(payload.startTimestamp / 1000)
        : Math.floor(payload.startTimestamp)
    }
    ...
    setDiscordPresence({
      ...
      largeImageKey,
      largeImageText: payload.title || 'flacr.',
      instance:       false,
    })
  })
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('settings:save', (_, data) => {
    saveFileSettings(data)
    if (autoUpdater) {
      autoUpdater.autoDownload = data?.autoUpdate === true
    }
    const checkerActive = data?.updateChecker !== false || data?.autoUpdate === true
    if (checkerActive) {
  ...
  // Start background loop on startup if checker/autoUpdate is active
  const startSettings = loadFileSettings() || {}
  const startCheckerActive = startSettings.updateChecker !== false || startSettings.autoUpdate === true
  if (startCheckerActive) {
    setTimeout(() => {
      startCheckingForUpdates()
    }, 8000)
  }
