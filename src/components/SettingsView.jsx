...
export function SettingsView({ servers, onSwitchServer, onRemoveServer, onRenameServer, onAddServer, switchingServerId }) {
  ...
  // ── Update status ──────────────────────────────────────────────────────────
  const { updateStatus, setUpdateStatus } = useUpdateStore()

  // Only check when nothing is known yet. The background loop already checks
  // on startup and every few hours, and the button below checks on demand.
  useEffect(() => {
    if (updateChecker && !updateStatus) window.electronAPI?.checkForUpdates?.()
