...
function Player({ session, onOpenNowPlaying, showQueue, onToggleQueue, showToast }) {
  ...
  const updateDiscordSeek = useCallback((seekPos) => {
    ...
    if (!s || !playing) return
    const coverId = s.ImageTags?.Primary ? s.Id : (s.AlbumId || s.Id)
    ipc.updatePresence({
      playing:        true,
      title:          s.Name,
      artist:         s.AlbumArtist || '',
      coverUrl:       imgUrl(sessionRef.current, coverId, 400),
  ...
  const lastPresenceSongId = useRef(null)
  ...
  useEffect(() => {
    ...
    if (!song || !isPlaying) {
      if (!song) lastPresenceSongId.current = null
      ipc.updatePresence({ playing: false })
      return
    }
    // A new track starts at 0. Only reuse the stored progress when the same
    // track resumes, for example after a pause.
    const sameSong = lastPresenceSongId.current === song.Id
    const elapsed  = sameSong ? useProgressStore.getState().progress : 0
    lastPresenceSongId.current = song.Id
    const coverId = song.ImageTags?.Primary ? song.Id : (song.AlbumId || song.Id)
    ipc.updatePresence({
      playing:        true,
      title:          song.Name,
      artist:         song.AlbumArtist || '',
      coverUrl:       imgUrl(session, coverId, 400),
      startTimestamp: Date.now() - Math.round(elapsed * 1000),
    })
  }, [song, isPlaying, discordRPC, session])
