...
import {
  imgUrl, verifySession, fetchAllSongs, fetchAlbums, fetchArtists,
  ...
  fetchRecentlyPlayed, fetchTopItems, fetchCurrentUser, logoutSession
} from '../utils/api'
...
function TitleBar({ isMaximized, isFullscreen }) {
  ...
  return (
    <div style={{
      height: '28px', flexShrink: 0, background: 'rgba(14,14,14,1)',
      borderBottom: 'none',
...
function UpdateBanner() {
  const { updateStatus, dismissed, setDismissed } = useUpdateStore()
  const { autoUpdate } = useSettingsStore()

  if (!updateStatus || dismissed) return null

  const isAvailable = updateStatus.type === 'available'
  ...
  // With automatic updates on, the download starts by itself, so the
  // "available" prompt is noise. The progress and the restart prompt still
  // matter in both modes.
  if (isAvailable && autoUpdate) return null
  ...
  if (isAvailable) {
    ...
    primaryAction = () => { window.electronAPI?.downloadUpdate?.() }
  } else if (isDownloaded) {
    text = 'A new version is ready to be installed.'
    primaryBtnLabel = 'Restart'
    primaryAction = () => { window.electronAPI?.installUpdate?.() }
  } else {
  ...
  return (
    <div style={{
      ...
      animation: 'bannerPopIn 0.3s cubic-bezier(0.16, 1, 0.3, 1) both',
    }}>
      {/* State icon */}
      <span style={{ display: 'flex', color: '#22c55e', flexShrink: 0 }}>
        {isDownloading ? (
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ animation: 'spin 1.2s linear infinite' }}>
      ...
      {/* Message + download progress bar */}
      <div style={{ flex: 1, minWidth: 0 }}>
        <span style={{ fontSize: '0.88rem', color: '#e2e8f0', fontWeight: 500, lineHeight: 1.4 }}>
          {text}
        </span>
        {isDownloading && (
          <div style={{ marginTop: '6px', width: '100%', height: '3px', borderRadius: '99px', background: 'rgba(255,255,255,0.08)', overflow: 'hidden' }}>
            <div style={{
...
function Library({ session: initialSession }) {
  ...
  const updateStatus = useUpdateStore(s => s.updateStatus)
  ...
  const handleAddServer = (newSession) => {
    ...
    const newServer = {
      id, name: newSession.serverName || newSession.serverUrl, url: newSession.serverUrl,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a0a', color: '#fff', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{
          width: sidebarWidth, flexShrink: 0, background: 'rgba(14,14,14,1)',
            ...
            >
              ...
              {hasUpdate && (
                isSidebarRetracted ? (
                  ...
                  <span style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center' }} title="Update available">
